...
const URI = "http://localhost:3001/usuarios/"; //ruta para hacer las peticiones

const Login = () => {
  const context = useContext(ShopContext);
  const navigate = useNavigate();
  const navigateRegister = () => {
    //redirige hacia el registro
    navigate(`/register`);
  };
  ...
  const navigateEditInventory = () => {
    //redirige hacia el editor del inventario
  ...
  const getUsers = async () => {
    //aqui se soolicitan los usuarios
    const res = await axios.get(URI);
    setUsers(res.data);
  };

  const compare = () => {
    //funcion para comparar si la entrada del usuario es igual a un usuario ya existente y poder logearse
    if (users.find((e) => e.nombre === entrada && e.password === entradaP))
      return true;
    else return false;
  };
  ...
  return (
    <div className="login-form">
      ...
      <form onSubmit={compare}>
        ...
        {" "}
        {/*al enviar el formulario se llama la funcion compare */}
        <input
        ...
        <input
          ...
          value="Login"
          onClick={(e) => {
            //al darle click al si compare es true osea que el login tuvo exito
            e.preventDefault();
            if (compare()) {
              //preguntasi la comparacion tuvo exito
              if (entrada === "admin") {
                //si el nombre de la entrada
              ...
              } else navigateShopAddtoCart(); //de lo contrario es un usuario normal por lo que envia al shop con los botones
      ...
      {/*se lleva al registro si se le da click */}
    </div>
  );
};
